...
class SpotIM_Admin {
    ...
    public static function admin_notice() {
        ...
        if ( 'closed' === get_default_comment_status() ) {
            printf(
                esc_html__( '%1$sTo properly run %2$sSpot.IM%3$s please visit your sites %4$sDiscussion Settings%5$s and turn on "%2$sAllow people to post comments on new articles%3$s". %6$s', 'ow' ),
    ...
    public static function import_callback() {
        ...
        if ( empty( $spot_id ) ) {
            $import->response( array(
                'status'  => 'error',
                'message' => esc_html__( 'Spot ID is missing.', 'ow' )
            ) );
